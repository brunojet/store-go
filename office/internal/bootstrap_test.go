...
func TestBootstrap_TipoCategoria_List(t *testing.T) {
	...
	// create a TipoCategoria directly using GORM via the repo's DB
	...
	// Unfortunately the repo does not expose DB; so use service List/assume empty then rely on insertion via NewCategoriaRepo

	// Instead insert using the repo implementation by calling NewTipoCategoriaRepo with a new connection
